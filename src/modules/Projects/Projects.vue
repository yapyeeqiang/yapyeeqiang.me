...
const visitGitHub = (url: string) => {
    window.open(url)
}

onMounted(async () => {
    startLoading()
    const result = await gh.queryRepos('yapyeeqiang')
    ...
    <MainLayout>
        <div class="max-w-5xl mx-auto">
            <div class="space-y-4">
                ...
                <div v-if="isLoading">
                    <p class="flex items-center space-x-2 text-gray-500 font-light animate-pulse">
                        <span>Loading</span>
                ...
                <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <Card @click="visitGitHub(repo.url)" class="relative" v-for="repo in repos" :key="repo.id">
                        <div>
